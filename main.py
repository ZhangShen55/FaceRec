...
async def lifespan(app: FastAPI):
    # ================= 启动 (Startup) =================
    log.info("System Startup: Initializing resources...")

    try:
        await db.command({"ping": 1})
        log.debug("MongoDB ping ok")
    except Exception as e:
        log.exception("MongoDB ping failed: %s", e)

    # 2. Dlib 进程池初始化 (注入到 ai_engine)
    # 确保 max_workers 设置合理 (建议 1 或 2，防止内存爆炸)
    # 当前 settings.thread.max_workers 建议设置为 2
    log.info(f"Initializing Dlib Process Pool with {MAX_WORKERS} workers...")
...
if __name__ == "__main__":
    ...
    # OMP_NUM_THREADS=1 uvicorn app.main:app --host 0.0.0.0 --port 8003 --workers 4