...
#        /data/facerecapi/{media/person_photos,logs}
#   2. 从仓库 app/config.toml 生成容器版 /data/facerecapi/config.toml
#      自动改写 5 处:
#        - [db].host                                   → facerec-mongo
#        - [redis].host                                → facerec-redis
#        - [face_detection.insightface].model_path     → /srv/app/ai_models
#        - [face_detection.insightface].gpu_id         → 0
#        - [gpu].gpu_id                                → 0
#   3. 模型已在 facerecapi 镜像构建时打包,目标机无需单独准备模型目录
#
# 注意：
#   - 不创建 docker network、不动任何容器（compose 会自管 facerec-network）
#   - 不写入业务数据
...
info "    /data/mongo /data/redis /data/facerecapi/{media,logs} 已就绪"

# ---------- 2) 生成容器版 config.toml ----------
TARGET_CFG="${DATA_FACEREC}/config.toml"
if [ -f "${TARGET_CFG}" ] && [ "${FORCE_CONFIG}" -eq 0 ]; then
    warn "2/2 目标已存在: ${TARGET_CFG}（保留不动）"
    ...
    if [ -f "${TARGET_CFG}" ] && [ "${FORCE_CONFIG}" -eq 1 ]; then
        ...
        warn "2/2 已备份旧配置: ${backup}"
    fi
    info "2/2 生成容器版 config.toml → ${TARGET_CFG}"
    cp "${TEMPLATE_CFG}" "${TARGET_CFG}"

    # 改写策略:用 sed 精准匹配 key,中间值用 [^"]* 避免贪婪吞注释
    sed -i -E 's|^(host[[:space:]]*=[[:space:]]*")[^"]*(".*#[[:space:]]*数据库地址.*)$|\1facerec-mongo\2|' "${TARGET_CFG}"
    sed -i -E 's|^(host[[:space:]]*=[[:space:]]*")[^"]*(".*#[[:space:]]*Redis[[:space:]]*服务器地址.*)$|\1facerec-redis\2|' "${TARGET_CFG}"
...
info "    docker compose up -d           # 一键拉起 facerec-mongo + facerec-redis + facerec-api"
info "    docker compose logs -f facerec-api"
info "============================================================"
